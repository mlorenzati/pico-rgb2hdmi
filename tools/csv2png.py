...
# Report format
print(filame, "is a ", w, "*", h+1, " with ", wordSize, " bits color density")
...
for row in range(h):
    for col in range(w):
        ...
        if  (wordSize == 16):
            ...
            print('file formatting error, neither 8 or 16 bits hexa, comma separated')
            exit(1) 
...
Image.fromarray(rgb888array).resize((w, aspectRatio43Height), resample=Image.NEAREST).save(outputFile)